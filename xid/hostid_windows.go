...
func readPlatformMachineID() (string, error) {
	...
	err = syscall.RegOpenKeyEx(syscall.HKEY_LOCAL_MACHINE, subKey, 0, syscall.KEY_READ|syscall.KEY_WOW64_64KEY, &h)
	if err != nil {
		return "", err
	}
	...
	var valType uint32
	subKey, err = syscall.UTF16PtrFromString(`MachineGuid`)
	if err != nil {
		return "", err
	}
	err = syscall.RegQueryValueEx(h, subKey, nil, &valType, (*byte)(unsafe.Pointer(&regBuf[0])), &bufLen)
	if err != nil {
		return "", err
	}
