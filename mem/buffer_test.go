...
func check(t *testing.T, testname string, buf *FakeIO, s string) {
	bytes := buf.Bytes()
	str := buf.String()
...
func fillString(t *testing.T, testname string, buf *FakeIO, s string, n int, fus string) string {
...
// The initial contents of buf corresponds to the string s;
// the result is the final contents of buf returned as a string.
func fillBytes(t *testing.T, testname string, buf *FakeIO, s string, n int, fub []byte) string {
	check(t, testname+" (fill 1)", buf, s)
	for ; n > 0; n-- {
		m, err := buf.Write(fub)
		if m != len(fub) {
...
func TestNewBuffer(t *testing.T) {
	buf := NewFakeIO(testBytes)
	check(t, "NewFakeIO", buf, testString)
}

func TestNewBufferString(t *testing.T) {
	buf := NewFakeIOString(testString)
	check(t, "NewFakeIOString", buf, testString)
}

// Empty buf through repeated reads into fub.
// The initial contents of buf corresponds to the string s.
func empty(t *testing.T, testname string, buf *FakeIO, s string, fub []byte) {
...
// Make sure that an empty FakeIO remains empty when
...
func TestReadFromNegativeReader(t *testing.T) {
	...
	defer func() {
		...
		case nil:
			t.Fatal("bytes.FakeIO.ReadFrom didn't panic")
		case error:
			// this is the error string of errNegativeRead
			wantError := "bytes.FakeIO: reader returned negative count from Read"
...
var readBytesTests = []struct {
	...
	{"empty", "", 0, []string{""}, io.EOF},
	{"byte1", "a\x00", 0, []string{"a\x00"}, nil},
	{"byte2", "abbbaaaba", 'b', []string{"ab", "b", "b", "aaab"}, nil},
	{"byte3", "hello\x01world", 1, []string{"hello\x01"}, nil},
	{"byte4", "foo\nbar", 0, []string{"foo\nbar"}, io.EOF},
	{"byte5 by LF", "alpha\nbeta\ngamma\n", '\n', []string{"alpha\n", "beta\n", "gamma\n"}, nil},
	{"byte6 by LF", "alpha\nbeta\ngamma", '\n', []string{"alpha\n", "beta\n", "gamma"}, io.EOF},
}
...
func TestReadBytes(t *testing.T) {
	for _, test := range readBytesTests {
		buf := NewFakeIOString(test.buffer)
		var err error
		...
		if err != test.err {
			t.Errorf("[%s] expected error %v, got %v", test.name, test.err, err)
		}
	}
}
...
func TestGrowOverflow(t *testing.T) {
	...
	buf := NewFakeIO(make([]byte, 1))
	const maxInt = int(^uint(0) >> 1)
	buf.Grow(maxInt)
}

// Was a bug: used to give EOF reading empty slice at EOF. (fixed)
...
func TestBufferGrowth(t *testing.T) {
	...
	cap1 := b.Cap()
	// (*FakeIO).grow allows for 2x capacity slop before sliding,
...
func BenchmarkWriteRune(b *testing.B) {
	...
	buf := NewFakeIO(make([]byte, n*utf8.UTFMax))
	for i := 0; i < b.N; i++ {
		buf.Reset()
		for i := 0; i < n; i++ {
			buf.WriteRune(r)
