...
func TestReaderAt(t *testing.T) {
	...
	tests := []struct {
		...
		{11, 10, "", io.EOF},
		{0, 0, "", nil},
		{-1, 0, "", "FakeIO.Multi.ReadAt: negative offset"},
	}
	for i, tt := range tests {
		b := make([]byte, tt.n)
...
var UnreadRuneErrorTests = []struct {
	...
	{"Read", func(r *FakeIO) { r.Read([]byte{0}) }},
	{"ReadByte", func(r *FakeIO) { r.ReadByte() }},
	{"UnreadRune", func(r *FakeIO) { r.UnreadRune() }},
	{"Seek", func(r *FakeIO) { r.Seek(0, io.SeekCurrent) }},
	{"WriteTo", func(r *FakeIO) { r.WriteTo(&FakeIO{}) }},
}

func TestUnreadRuneError(t *testing.T) {
	for _, tt := range UnreadRuneErrorTests {
		reader := NewFakeIO([]byte("0123456789"))
...
func TestReaderCopyNothing(t *testing.T) {
	...
	with.n, with.err = io.Copy(discard, NewFakeIO(nil))
	withOut.n, withOut.err = io.Copy(discard, justReader{NewFakeIO(nil)})
	if with != withOut {
...
func TestReaderZero(t *testing.T) {
	...
	if n, err := (&FakeIO{}).Read(nil); n != 0 || err != io.EOF {
		t.Errorf("Read: got %d, %v; want 0, io.EOF", n, err)
	}

	if n, err := (&FakeIO{}).ReadAt(nil, 11); n != 0 || err != io.EOF {
		t.Errorf("ReadAt: got %d, %v; want 0, io.EOF", n, err)
	}

	if b, err := (&FakeIO{}).ReadByte(); b != 0 || err != io.EOF {
		t.Errorf("ReadByte: got %d, %v; want 0, io.EOF", b, err)
	}

	if ch, size, err := (&FakeIO{}).ReadRune(); ch != 0 || size != 0 || err != io.EOF {
		t.Errorf("ReadRune: got %d, %d, %v; want 0, 0, io.EOF", ch, size, err)
	}

	if offset, err := (&FakeIO{}).Seek(11, io.SeekStart); offset != 11 || err != nil {
		t.Errorf("Seek: got %d, %v; want 11, nil", offset, err)
	}

	if s := (&FakeIO{}).Size(); s != 0 {
		t.Errorf("Size: got %d, want 0", s)
	}

	if (&FakeIO{}).UnreadByte() == nil {
		t.Errorf("UnreadByte: got nil, want error")
	}

	if (&FakeIO{}).UnreadRune() == nil {
		t.Errorf("UnreadRune: got nil, want error")
	}

	if n, err := (&FakeIO{}).WriteTo(io.Discard); n != 0 || err != nil {
